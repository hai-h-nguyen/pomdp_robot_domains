...
class CloseLoopPomdpBlockPickingEnv(CloseLoopEnv):
  ...
  def reset(self, target_obj_idx, noise=False):
    ...
    if target_obj_idx == 0:
      self._generateShapes(constants.CUBE, 1, pos=[[0.5 + noise_x1, 0.12 + noise_y1, 0.025]], cube_color='red', random_orientation=self.random_orientation, rot=rot_1)
      self._generateShapes(constants.CUBE, 1, pos=[[0.5 + noise_x2, 0.0 + noise_y2, 0.025]], cube_color='blue', random_orientation=self.random_orientation, rot=rot_2)
    elif (target_obj_idx == 1):
      self._generateShapes(constants.CUBE, 1, pos=[[0.5 + noise_x1, 0.12 + noise_y1, 0.025]], cube_color='blue', random_orientation=self.random_orientation, rot=rot_1)
      self._generateShapes(constants.CUBE, 1, pos=[[0.5 + noise_x2, 0.0 + noise_y2, 0.025]], cube_color='red', random_orientation=self.random_orientation, rot=rot_2)
